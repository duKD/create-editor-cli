...
import require$$6, { renameSync, unlinkSync } from 'fs';
...
const emptyDir = (dir) => {
  ...
  for (const file of fs$2.readdirSync(dir)) {
    fs$2.rmSync(path$1.resolve(dir, file), { recursive: true, force: true });
  }
};
...
async function createProject(tempFilePath, type) {
  try {
    ...
    const writer = fs$2.createWriteStream(tempFilePath);
...
var require = fileSystem.require = function () {
...
function unzip(targetDir, tempFilePath, projectName, type) {
  try {
    ...
    const oldFilePath = require$$1$1.join(targetDir, templateName[type]);
    const newFilePath = require$$1$1.join(targetDir, projectName);
...
async function init() {
  const questions = [
    ...
    {
      type: (projectName2) => {
        let filePath2 = path$1.join(cwd, projectName2);
        return !fs$2.existsSync(filePath2) ? null : "select";
      },
      name: "overwrite",
  ...
  const filePath = path$1.join(cwd, projectName);
  if (overwrite === "yes") {
    emptyDir(filePath);
  }
  console.log("\u5F00\u59CB\u4E0B\u8F7D\u6A21\u7248...");
  const templateName = projectName + ".zip";
  const tempFilePath = path$1.join(cwd, templateName);
