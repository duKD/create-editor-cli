...
const fs__default = /*#__PURE__*/_interopDefaultCompat(fs$2);
...
const emptyDir = (dir) => {
  ...
  for (const file of fs__default.readdirSync(dir)) {
    fs__default.rmSync(path__default.resolve(dir, file), { recursive: true, force: true });
  }
};
...
function requireWrap$1 () {
	if (hasRequiredWrap$1) return wrap$2;
	hasRequiredWrap$1 = 1;
	/**
...
function requireWrap () {
	if (hasRequiredWrap) return wrap$1;
	hasRequiredWrap = 1;
...
function requireToggle () {
	...
	const { cursor, erase } = requireSrc$1();
...
var Stream$2 = stream__default.Stream;
var util$3 = require$$1__default;
...
var http$1 = require$$3__default;
var https$1 = require$$4__default;
var parseUrl$2 = require$$0__default$1.parse;
...
const URLSearchParams = require$$0__default$1.URLSearchParams;
...
var parseUrl$1 = require$$0__default$1.parse;
...
function requireNode () {
	...
	hasRequiredNode = 1;
	(function (module, exports) {
		const tty = require$$0__default$3;
...
const isBrotliSupported = utils$2.isFunction(zlib__default.createBrotliDecompress);
...
const httpAdapter = isHttpAdapterSupported && function httpAdapter(config) {
  return wrapAsync(async function dispatchHttpRequest(resolve, reject, onDone) {
    ...
    const emitter = new require$$2.EventEmitter();
    ...
    if (utils$2.isSpecCompliantForm(data)) {
      ...
      data = stream__default.Readable.from(readBlob$1(data));
    ...
    if (data && (onUploadProgress || maxUploadRate)) {
      ...
      data = stream__default.pipeline([data, new AxiosTransformStream$1({
    ...
    if (config.transport) {
      ...
      transport = isHttpsRequest ? require$$4__default : require$$3__default;
    } else {
    ...
    req = transport.request(options, function handleResponse(res) {
      ...
      responseStream = streams.length > 1 ? stream__default.pipeline(streams, utils$2.noop) : streams[0];

      const offListeners = stream__default.finished(responseStream, () => {
        offListeners();
        onFinished();
      });
...
const progress$1 = /*@__PURE__*/getDefaultExportFromCjs(progress);

const zipUrl = {
  vue: "https://github.com/duKD/vue3-picture-editor/archive/refs/heads/main.zip",
  react: "https://github.com/duKD/react-picture-editor/archive/refs/heads/main.zip"
};
const codeError = "ECONNREFUSED";
async function createProject(tempFilePath, type) {
  try {
    const res = await axios$1.get(zipUrl[type], {
      responseType: "stream"
    });
    const writer = fs__default.createWriteStream(tempFilePath);
    res.data.pipe(writer);
    const totalLength = parseInt(res.headers["content-length"], 10) || 454306;
    const progressBar = new progress$1(":bar :percent ", {
      complete: "=",
      incomplete: " ",
      width: 100,
      total: totalLength
...
const pth$2 = require$$1__default$1;
...
const pth$1 = require$$1__default$1;
...
var inflater = function (/*Buffer*/ inbuf, /*number*/ expectedLength) {
    var zlib = zlib__default;
...
const { randomFillSync } = require$$0__default$4;
...
var zipEntry = function (/*Buffer*/ input) {
    var _centralHeader = new Headers$1.EntryHeader(),
...
var zipFile = function (/*Buffer|null*/ inBuffer, /** object */ options) {
    ...
    function readEntries() {
        loadedEntries = true;
        entryTable = {};
        if (mainHeader.diskEntries > (inBuffer.length - mainHeader.offset) / Utils$1.Constants.CENHDR) {
            throw new Error(Utils$1.Errors.DISK_ENTRY_TOO_LARGE);
        }
...
const pth = require$$1__default$1;
...
function unzip(targetDir, tempFilePath, projectName, type) {
  try {
    const zip = new AdmZip(tempFilePath);
    zip.extractAllTo(targetDir, true);
    const oldFilePath = require$$1__default$1.join(targetDir, templateName[type]);
    const newFilePath = require$$1__default$1.join(targetDir, projectName);
    require$$6.renameSync(oldFilePath, newFilePath);
    require$$6.unlinkSync(tempFilePath);
  } catch (error) {
...
async function init() {
  const questions = [
    ...
    {
      type: (projectName2) => isValidPackageName(projectName2) ? null : "text",
      name: "projectName",
      message: "\u540D\u79F0\u683C\u5F0F\u4E0D\u7B26\uFF0C\u8BF7\u91CD\u65B0\u8F93\u5165\u9879\u76EE\u540D\uFF01"
    },
    {
      type: (projectName2) => {
        let filePath2 = path__default.join(cwd, projectName2);
        return !fs__default.existsSync(filePath2) ? null : "select";
      },
      name: "overwrite",
  ...
  const { type, projectName, overwrite } = await prompts$1(questions);
  const filePath = path__default.join(cwd, projectName);
  if (overwrite === "yes") {
    emptyDir(filePath);
  }
  console.log("\u5F00\u59CB\u4E0B\u8F7D\u6A21\u7248...");
  const templateName = projectName + ".zip";
  const tempFilePath = path__default.join(cwd, templateName);
